			...
			content = @Content(
				...
				examples = {
					@ExampleObject(
						...
						value = "{\"code\":\"INVALID_PARAMETER\", \"message\":\"Latitude must not be null\"}"
					),
					@ExampleObject(
						name = "Longitude must not be null",
						value = "{\"code\":\"INVALID_PARAMETER\", \"message\":\"Longitude must not be null\"}"
					),
					@ExampleObject(
						name = "Latitude out of range",
						value = "{\"code\":\"INVALID_PARAMETER\", \"message\":\"Latitude must be at least -90 degrees\"}"
					),
					@ExampleObject(
						name = "Longitude out of range",
						value = "{\"code\":\"INVALID_PARAMETER\", \"message\":\"Longitude must be at most 180 degrees\"}"
					),
					@ExampleObject(
						name = "Invalid coordinates",
						value = "{\"code\":\"INVALID_COORDINATES\", \"message\":\"Invalid location coordinates\"}"
					),
					@ExampleObject(
						name = "Date out of range",
						value = "{\"code\":\"DATE_OUT_OF_RANGE\", \"message\":\"Date is out of range (maximum +3 days)\"}"
					)
				}
			)
		),
			...
			content = @Content(
				...
				examples = {
					@ExampleObject(
						...
						value = "{\"code\":\"SERVICE_UNAVAILABLE\", \"message\":\"An error occurred while processing weather service\"}"
					)
				}
			)
		)
